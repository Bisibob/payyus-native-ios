...
public class AutoCompleteTextField:UITextField {
    ...
    public var autoCompleteAttributes:[NSAttributedStringKey: Any]?
...
//    public var autoCompleteTableWidth: CGFloat?
    ...
    public func updateFrame(){
        if let tableView = autoCompleteTableView {
            ...
            frame.size.width = UIScreen.main.bounds.size.width - (self.frame.origin.x * 2)
        ...
        if enableAttributedText{
            ...
            if let autoCompleteStrings = autoCompleteStrings, let autoCompleteAttributes = autoCompleteAttributes {
                for i in 0..<autoCompleteStrings.count{
                    ...
                    let range = str.range(of: text!, options: NSString.CompareOptions.caseInsensitive)
...
extension AutoCompleteTextField: UITableViewDataSource, UITableViewDelegate {
    ...
    public func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        if let selectedText = cell?.textLabel?.text {
            ...
            onSelect(selectedText, indexPath as NSIndexPath as NSIndexPath)
        }
    ...
    public func tableView(_ tableView: UITableView, willDisplay cell: UITableViewCell, forRowAt indexPath: IndexPath) {
        if cell.responds(to: #selector(setter: UITableViewCell.separatorInset)){
            cell.separatorInset = self.autoCompleteSeparatorInset
        }
